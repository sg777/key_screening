#!/usr/bin/env python3
"""
Script to generate ECDSA keys (NIST P-256) in parallel and find a key whose SHA-256 hash is a superset
of the hardcoded hash 13080e6066508e80e0228f19082e83e9094509a0b0234804406248c1cd208518 (172 zeros, 84 ones),
...
def count_ones(hash_hex: str) -> int:
    """Count the number of 1's in the binary representation of a 256-bit SHA-256 hash."""
...
def is_superset(new_hash_hex: str, stored_hash_hex: str) -> bool:
    ...
    new_binary = bin(int(new_hash_hex, 16))[2:].zfill(256)
    stored_binary = bin(int(stored_hash_hex, 16))[2:].zfill(256)
    return all(stored_binary[i] == '0' or new_binary[i] == '1' for i in range(256))

def generate_and_check_superset_key(trial_counter: mp.Value, lock: mp.Lock, target_hash: str, stop_event: mp.Event) -> None:
    """Generate an ECDSA key and check if its SHA-256 hash is a superset of the target hash."""
    while not stop_event.is_set():
        ...
        hash_obj = hashlib.sha256(pubkey_der)
        ...
        if is_superset(hash_hex, target_hash):
            ...
            with lock:
                with open('sha256.superhashes.txt', 'a') as f:
...
def main():
    """Run key generation in parallel to find a superset of the hardcoded SHA-256 hash."""
    target_hash = "13080e6066508e80e0228f19082e83e9094509a0b0234804406248c1cd208518"  # 172 zeros, 84 ones
    ...
    print(f"Generating ECDSA keys (NIST P-256) in parallel ({num_processes} processes) to find SHA-256 hashes that are supersets of {target_hash}...")
    processes = [
