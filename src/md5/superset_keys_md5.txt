#!/usr/bin/env python3
"""
Script to generate ECDSA keys (NIST P-256) in parallel and find a key whose MD5 hash is a superset
of the hardcoded hash 825020c00126288101a3b11424208009 (96 zeros, 32 ones),
...
def count_ones(hash_hex: str) -> int:
    """Count the number of 1's in the binary representation of a 128-bit MD5 hash."""
...
def is_superset(new_hash_hex: str, stored_hash_hex: str) -> bool:
    ...
    new_binary = bin(int(new_hash_hex, 16))[2:].zfill(128)
    stored_binary = bin(int(stored_hash_hex, 16))[2:].zfill(128)
    return all(stored_binary[i] == '0' or new_binary[i] == '1' for i in range(128))

def generate_and_check_superset_key(trial_counter: mp.Value, lock: mp.Lock, target_hash: str, stop_event: mp.Event) -> None:
    """Generate an ECDSA key and check if its MD5 hash is a superset of the target hash."""
    while not stop_event.is_set():
        ...
        hash_obj = hashlib.md5(pubkey_der)
        ...
        if is_superset(hash_hex, target_hash):
            ...
            with lock:
                with open('superset_keys_md5.txt', 'a') as f:
...
def main():
    """Run key generation in parallel to find a superset of the hardcoded MD5 hash."""
    target_hash = "825020c00126288101a3b11424208009"  # 96 zeros, 32 ones
    ...
    print(f"Generating ECDSA keys (NIST P-256) in parallel ({num_processes} processes) to find MD5 hashes that are supersets of {target_hash}...")
    processes = [
